# Deleting from the BeaST by Edwin Bartlett
...
- Find the in-order successor (minimum node in the right subtree) and its parent
- Copy successor's key to the current node (replacement)
- Update 'current' and 'parent' for the remaining logic to delete the successor's original location.
